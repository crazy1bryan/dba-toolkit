Param(
	...
	[string]$primaryReadScale,
	[string]$primaryZoneRedundant,
	[string]$secondaryResourceGroup,
	[string]$secondaryLocation,
	[string]$secondaryServer,
	[string]$secondaryEdition,
	[string]$secondaryServiceObjective,
	[string]$secondaryReadScale,
	[string]$secondaryZoneRedundant
)
...
# Installation login
Invoke-Sqlcmd -Query "IF EXISTS (SELECT 1 FROM sys.sql_logins WHERE name = '$InstallLogin') DROP LOGIN [$InstallLogin] CREATE LOGIN [$InstallLogin] WITH PASSWORD = '$InstallPassword';" -ServerInstance $primaryServer_FQ -Database "master" -Username $administratorLogin -Password $administratorPassword

# Application login
Invoke-Sqlcmd -Query "IF EXISTS (SELECT 1 FROM sys.sql_logins WHERE name = '$applicationLogin') DROP LOGIN [$applicationLogin] CREATE LOGIN [$applicationLogin] WITH PASSWORD = '$applicationPassword';" -ServerInstance $primaryServer_FQ -Database "master" -Username $administratorLogin -Password $administratorPassword
 
# Installation user/permissions
Invoke-Sqlcmd -Query "IF EXISTS (SELECT 1 FROM sysusers WHERE name = '$InstallLogin') DROP USER [$InstallLogin] CREATE USER [$InstallLogin] FOR LOGIN [$InstallLogin];" -ServerInstance $primaryServer_FQ -Database $databaseName -Username $administratorLogin -Password $administratorPassword
